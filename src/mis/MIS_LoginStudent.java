...
public class MIS_LoginStudent extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
					MIS_LoginStudent frame = new MIS_LoginStudent();
	 ...
	 * Create the frame.
	 */
	public MIS_LoginStudent() {
		connection = DBConnector.dbConnector();
		setIconImage(Toolkit.getDefaultToolkit().getImage(MIS_LoginStudent.class.getResource("/Resources/Custo.Man.Christmas.Folder.Library.ico.png")));
			...
			public void keyPressed(KeyEvent e) {
				if(e.getKeyCode()==KeyEvent.VK_ENTER)
				{
					...
					else
					{
						try
						{
							String Query = "select * from StudentsMF where RegNo=? and Password=?";
							...
							while(rs.next())
							{
								byte[] img = rs.getBytes(20);
								//ImageIcon pp = new ImageIcon(img);
								BufferedImage bufimg;
								try {
									bufimg = ImageIO.read(new ByteArrayInputStream(img));
									MIS_HomeStudent frame = new MIS_HomeStudent();
									Image prof = resizeImage(bufimg, frame.profilePicture.getWidth(), frame.profilePicture.getHeight());
									frame.lblSid.setText(rs.getString(1));
									frame.lblSName.setText(rs.getString(2));
									ImageIcon profilePic = new ImageIcon(prof);
									frame.profilePicture.setIcon(profilePic);
									frame.setUndecorated(true);
									frame.setVisible(true);
									count++;
