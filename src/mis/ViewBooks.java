...
public class ViewBooks extends JFrame {
	...
	public ViewBooks() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(ViewBooks.class.getResource("/Resources/Custo.Man.Christmas.Folder.Library.ico.png")));
		...
		String[] bType = new String[] {"Available", "Issued", "All"};
		...
		JComboBox cmbBType = new JComboBox(bType);
		cmbBType.setFont(new Font("Ubuntu", Font.PLAIN, 11));
		cmbBType.setForeground(Color.WHITE);
		cmbBType.setBackground(Color.GRAY);
		cmbBType.setBorder(BorderFactory.createEmptyBorder());
		cmbBType.setBounds(508, 579, 133, 20);
		panel.add(cmbBType);
		...
		btnView.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try
				{
					if(cmbBType.getSelectedItem().toString()=="Available")
					{
						String Query = "select * from LibBooks where Status ='Available'";
					...
					else if(cmbBType.getSelectedItem().toString()=="Issued")
					{
						String Query = "select * from LibBooks where Status ='Issued'";
						PreparedStatement pst = connection.prepareStatement(Query);
						ResultSet rs = pst.executeQuery();
						table.setModel(DbUtils.resultSetToTableModel(rs));
						rs.close();
					...
					else if(cmbBType.getSelectedItem().toString()=="All")
					{
		...
		btnView.setBounds(712, 577, 89, 23);
		panel.add(btnView);
